...
uint8_t  Adafruit_SI1145::read8(uint8_t reg) {
  ...
  if (use_sercom2) {
    ...
    myWire.requestFrom((uint8_t)_addr, (uint8_t)1);
    return myWire.read();
  } else {
...
uint16_t Adafruit_SI1145::read16(uint8_t a) {
  ...
  if (use_sercom2) {
    ...
    myWire.requestFrom(_addr, (uint8_t)2);// send data n-bytes read
    ret = myWire.read(); // receive DATA
    ret |= (uint16_t)myWire.read() << 8; // receive DATA
  } else {
...
void Adafruit_SI1145::write8(uint8_t reg, uint8_t val) {
  if (use_sercom2) {
    myWire.beginTransmission(_addr); // start transmission to device
    myWire.write(reg); // sends register address to write
    myWire.write(val); // sends value
    myWire.endTransmission(); // end transmission
  } else {
    Wire.beginTransmission(_addr); // start transmission to device
