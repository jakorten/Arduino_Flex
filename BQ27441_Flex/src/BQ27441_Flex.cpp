/******************************************************************************
SparkFunBQ27441_Flex.cpp
BQ27441_Flex Arduino Library Main Source File
Jim Lindblom @ SparkFun Electronics
May 9, 2016
https://github.com/sparkfun/SparkFun_BQ27441_Arduino_Library

Implementation of all features of the BQ27441_Flex LiPo Fuel Gauge.
...
BQ27441_Flex::BQ27441_Flex(TwoWire *wire) : _deviceAddress(BQ72441_I2C_ADDRESS), _sealFlag(false), _userConfigControl(false)
{
	this->_wire = wire;
}

// Initializes I2C and verifies communication with the BQ27441_Flex.
bool BQ27441_Flex::begin(void)
{
	uint16_t deviceID = 0;

	//wire.begin(); // Initialize I2C master
	// JK: do not do this inside this library it is a bad practice and might freeze the i2c bus!!!

	deviceID = deviceType(); // Read deviceType from BQ27441_Flex
...
bool BQ27441_Flex::setCapacity(uint16_t capacity)
{
	// Write to STATE subclass (82) of BQ27441_Flex extended memory.
	// Offset 0x0A (10)
...
uint16_t BQ27441_Flex::voltage(void)
{
	return readWord(BQ27441_COMMAND_VOLTAGE);
}

// Reads and returns the specified current measurement
int16_t BQ27441_Flex::current(current_measure type)
{
	int16_t current = 0;
	switch (type)
	{
...
uint16_t BQ27441_Flex::capacity(capacity_measure type)
{
	uint16_t capacity = 0;
	switch (type)
	{
...
int16_t BQ27441_Flex::power(void)
{
	return (int16_t) readWord(BQ27441_COMMAND_AVG_POWER);
}

// Reads and returns specified state of charge measurement
uint16_t BQ27441_Flex::soc(soc_measure type)
{
	uint16_t socRet = 0;
	switch (type)
	{
...
uint8_t BQ27441_Flex::soh(soh_measure type)
{
...
uint16_t BQ27441_Flex::temperature(temp_measure type)
{
	uint16_t temp = 0;
	switch (type)
	{
...
bool BQ27441_Flex::GPOUTPolarity(void)
{
...
bool BQ27441_Flex::setGPOUTPolarity(bool activeHigh)
{
...
bool BQ27441_Flex::GPOUTFunction(void)
{
	uint16_t opConfigRegister = opConfig();
...
bool BQ27441_Flex::setGPOUTFunction(gpout_function function)
{
	uint16_t oldOpConfig = opConfig();
...
bool BQ27441_Flex::socFlag(void)
{
...
bool BQ27441_Flex::socfFlag(void)
{
	uint16_t flagState = flags();
...
bool BQ27441_Flex::pulseGPOUT(void)
{
...
uint16_t BQ27441_Flex::deviceType(void)
{
...
bool BQ27441_Flex::enterConfig(bool userControl)
{
...
bool BQ27441_Flex::exitConfig(bool resim)
{
...
uint16_t BQ27441_Flex::flags(void)
{
	return readWord(BQ27441_COMMAND_FLAGS);
}

// Read the CONTROL_STATUS subcommand of control()
uint16_t BQ27441_Flex::status(void)
{
...
// Check if the BQ27441_Flex-G1A is sealed or not.
bool BQ27441_Flex::sealed(void)
{
	uint16_t stat = status();
	return stat & BQ27441_STATUS_SS;
}

// Seal the BQ27441_Flex-G1A
bool BQ27441_Flex::seal(void)
{
	return readControlWord(BQ27441_CONTROL_SEALED);
}

// UNseal the BQ27441_Flex-G1A
bool BQ27441_Flex::unseal(void)
{
	// To unseal the BQ27441_Flex, write the key to the control
...
// Issue a soft-reset to the BQ27441_Flex-G1A
bool BQ27441_Flex::softReset(void)
{
	return executeControlWord(BQ27441_CONTROL_SOFT_RESET);
}

// Read a 16-bit command word from the BQ27441_Flex-G1A
uint16_t BQ27441_Flex::readWord(uint16_t subAddress)
{
	uint8_t data[2];
...
// Read a 16-bit subcommand() from the BQ27441_Flex-G1A's control()
uint16_t BQ27441_Flex::readControlWord(uint16_t function)
{
...
// Execute a subcommand() from the BQ27441_Flex-G1A's control()
bool BQ27441_Flex::executeControlWord(uint16_t function)
{
	uint8_t subCommandMSB = (function >> 8);
	uint8_t subCommandLSB = (function & 0x00FF);
	uint8_t command[2] = {subCommandLSB, subCommandMSB};
...
bool BQ27441_Flex::blockDataControl(void)
{
...
bool BQ27441_Flex::blockDataClass(uint8_t id)
{
	return i2cWriteBytes(BQ27441_EXTENDED_DATACLASS, &id, 1);
}

// Issue a DataBlock() command to set the data block to be accessed
bool BQ27441_Flex::blockDataOffset(uint8_t offset)
{
	return i2cWriteBytes(BQ27441_EXTENDED_DATABLOCK, &offset, 1);
}

// Read the current checksum using BlockDataCheckSum()
uint8_t BQ27441_Flex::blockDataChecksum(void)
{
	uint8_t csum;
...
uint8_t BQ27441_Flex::readBlockData(uint8_t offset)
{
	uint8_t ret;
...
bool BQ27441_Flex::writeBlockData(uint8_t offset, uint8_t data)
{
	uint8_t address = offset + BQ27441_EXTENDED_BLOCKDATA;
...
uint8_t BQ27441_Flex::computeBlockChecksum(void)
{
	uint8_t data[32];
...
bool BQ27441_Flex::writeBlockChecksum(uint8_t csum)
{
...
uint8_t BQ27441_Flex::readExtendedData(uint8_t classID, uint8_t offset)
{
	uint8_t retData = 0;
...
bool BQ27441_Flex::writeExtendedData(uint8_t classID, uint8_t offset, uint8_t * data, uint8_t len)
{
	if (len > 32)
		return false;
...
int16_t BQ27441_Flex::i2cReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	int16_t timeout = BQ72441_I2C_TIMEOUT;
	...
	while ((_wire->available() < count) && timeout--)
		delay(1);
	if (timeout)
	{
...
uint16_t BQ27441_Flex::i2cWriteBytes(uint8_t subAddress, uint8_t * src, uint8_t count)
{
	_wire->beginTransmission(_deviceAddress);
	_wire->write(subAddress);
	for (int i=0; i<count; i++)
...
BQ27441_Flex lipoGauge(TwoWire *wire); // Use lipo.[] to interact with the library in an Arduino sketch
